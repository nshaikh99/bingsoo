...
TEST(RequestCrudTest, PostRequest) {
    ...
    //ASSERT_TRUE(m_filesystem->file_exists(createdFile));
    //ASSERT_EQ(contents, requestBody);
}
/*
...
TEST(RequestCrudTest, GetFile) {
    ...
    http::response<http::string_body> response;
    // Create an instance of the request_crud class
    crud_request_handler crud = crud_request_handler("/mnt/crud", m_filesystem);
    ...
    // Call the handle_request function
    req_status status = crud.handle_request(request, response);
    // Assert the expected result
    ASSERT_TRUE(status);
